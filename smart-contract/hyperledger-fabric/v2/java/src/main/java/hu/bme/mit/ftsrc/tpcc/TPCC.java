...
package hu.bme.mit.ftsrc.tpcc;

import com.google.gson.Gson;

import hu.bme.mit.ftsrc.tpcc.entries.*;
import hu.bme.mit.ftsrc.tpcc.inputs.*;
import hu.bme.mit.ftsrc.tpcc.outputs.*;
import hu.bme.mit.ftsrc.tpcc.utils.*;
...
public class TPCC implements ContractInterface {
  ...
  public String doDelivery(Context ctx, String parameters) {
    ...
    try {
      ...
      //     // ctx, "info");
      //     LOGGER.info(
      //             "Could not find new order for District( " + params.w_id + "," + d_id + " )
      // skipping it");
      //     skipped += 1;
      //     continue;
      // }
      ...
      DoDeliveryOutput output =
          new DoDeliveryOutput(params.w_id, params.o_carrier_id, deliveredOrders);
  ...
  public String doNewOrder(Context ctx, String parameters) {
    ...
    try {
      ...
      final Customer customer =
          LedgerUtils.getCustomer(ctx, warehouse.w_id, district.d_id, params.c_id);
      LOGGER.info(
          "Customer "
              + params.c_id
              + " with w_id "
  ...
  public void initEntries(Context ctx) {
    ...
    try {
      ...
      LOGGER.info(
          "INNITIALIZED STOCK ENTRIES" + jsonStock + " , " + jsonStock2 + " and " + jsonStock3);
